...
static void
test_lexer_parses_simple_symbol(void) {
    hll_lexer lexer;
...
test_lexer_parses_number(void) {
    ...
    TEST_ASSERT(strncmp(lexer.input + lexer.next.offset, "123",
                        lexer.next.length) == 0);
...
test_lexer_parse_basic_syntax_multiple_tokens(void) {
    ...
    TEST_ASSERT(strncmp(lexer.input + lexer.next.offset, "cons",
                        lexer.next.length) == 0);
