...
struct hll_location_table {
  ...
  struct hll_location_entry *entries;
};
...
hll_value hll_read_ast(struct hll_reader *reader) __attribute__((nonnull));

struct hll_compiler_loc_stack_entry {
  uint32_t cu;
  uint32_t offset;
  uint32_t length;
};
...
struct hll_compiler {
  ...
  struct hll_compiler_loc_stack_entry *loc_stack;
